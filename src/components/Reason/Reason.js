...
import AOS from 'aos';
import './Reason.css';
import { reasons } from '../Reason/Reasons.js';
            ...
            <div className="md:flex md:flex-row md:justify-between md:items-center mx-auto w-[85%] space-x-10 mt-12">
                    {
                        reasons?.map((reason, index) => (
                            <div key={index} className=" border text-center w-[25%] px-3 py-7 h-[190px] space-y-2 rounded-[.3rem] shadow-md border-gray-100">
                                <img src={reason.icon} className="h-8 w-8 mx-auto"/>
                                <h3 className="text-[14px] font-bold text-[#226802]">{reason.header}</h3>
                                <p className="text-[12px] text-[#111111]">{reason.content}</p>
                            </div>
